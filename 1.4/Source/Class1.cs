...
{
    public class BiotechnologyMod : Mod
    {
        public BiotechnologyMod(ModContentPack content) : base(content)
        {
    ...
    public static class GenerateImpliedDefs_PreResolve_Patch
    {
        ...
        public static void Prefix()
        {
            ...
            {
                generated = true;
                List<ThingDef> allCultures = DefDatabase<ThingDef>.AllDefs.Where(x => (x.thingCategories?.Any(thingCategory => thingCategory.defName.StartsWith("BiotechnologyCategory")) ?? false)
&& x.comps.Any(compProperties => compProperties is CompProperties_Hatcher)).ToList();
                foreach (PawnKindDef animal in DefDatabase<PawnKindDef>.AllDefs.OrderBy(x => x.race.race.baseBodySize))
                {
                    ThingDef existingComp = allCultures.FirstOrDefault(x => x.GetCompProperties<CompProperties_Hatcher>().hatcherPawn == animal);
        ...
        {
            ThingDef capsuleDef = GetBasicCapsuleDef(animal);
            capsuleDef.thingCategories = new List<ThingCategoryDef>
                {
                    ThingCategoryDef.Named("BiotechnologyCategory_Pet")
                };
            RecipeDef recipeDef = GetRecipeDefForPet(capsuleDef, animal);
            capsuleDef.PostLoad();
            recipeDef.PostLoad();
        ...
        {
            RecipeDef def = GetBasicRecipeDef(capsule, animal);
        ...
        private static void PopulateCulturedInsect(PawnKindDef animal)
        {
            ...
            RecipeDef recipeDef = GetRecipeDefForInsect(capsuleDef, animal);
            capsuleDef.PostLoad();
            recipeDef.PostLoad();
            DefDatabase<ThingDef>.Add(capsuleDef);
            DefDatabase<RecipeDef>.Add(recipeDef);
        ...
        private static void PopulateCulturedSmallAnimal(PawnKindDef animal)
        {
            ...
            RecipeDef recipeDef = GetRecipeDefForSmallAnimal(capsuleDef, animal);
            capsuleDef.PostLoad();
            recipeDef.PostLoad();
            DefDatabase<ThingDef>.Add(capsuleDef);
            DefDatabase<RecipeDef>.Add(recipeDef);
        ...
        private static void PopulateCulturedMediumAnimal(PawnKindDef animal)
        {
            ...
            RecipeDef recipeDef = GetRecipeDefForMediumAnimal(capsuleDef, animal);
            capsuleDef.PostLoad();
            recipeDef.PostLoad();
            DefDatabase<ThingDef>.Add(capsuleDef);
            DefDatabase<RecipeDef>.Add(recipeDef);
        ...
        private static void PopulateCulturedLargeAnimal(PawnKindDef animal)
        {
            ...
            RecipeDef recipeDef = GetRecipeDefForLargeAnimal(capsuleDef, animal);
            capsuleDef.PostLoad();
            recipeDef.PostLoad();
            DefDatabase<ThingDef>.Add(capsuleDef);
            DefDatabase<RecipeDef>.Add(recipeDef);
